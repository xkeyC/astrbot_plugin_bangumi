...
class SubscriptionService:
    ...
    async def subscribe(self, group_id: str, query: str) -> str:
        ...
        try:
            # 1. 匹配条目
            error_msg, subject_info = await self.service.match_subscribable_subject(query)
            if error_msg:
                return error_msg
            if not subject_info:
...
如有更新将推送到本群。"
    ...
    async def check_updates(self):
        ...
        for subject in subjects:
            try:
                # 获取最新集数
                latest_episode = await self.service.get_latest_episode(int(subject.subject_id))
                ...
                try:
                    ...
                    if image_base64:
                        latest_episode.image_url = f"data:image/png;base64,{image_base64}"
                except Exception as e:
                ...
                if latest_episode.ep > subject.current_episode:
                    ...
                    # 更新数据库
                    self.storage.update_subject_episode(subject.subject_id, latest_episode.ep)
                    
                    # 发送通知
                    await self._notify_subscribers(latest_episode, subject.subject_id, subject.name)
            except Exception as e:
                logger.error(f"更新番剧《{subject.name}》失败: {e}")

    async def _notify_subscribers(self, episode: Episode, subject_id: str, subject_name: str):
        """
        渲染并发送更新通知。
        """
        ...
        if base64_image:
            ...
            chain = chain.text(f"🔔 番剧《{subject_name}》更新啦！
